...
// customers class
require_once BASE_PATH . '/lib/Formats/Customer.php';
...
$db = getDbInstance();
$select = array('customer_email', 'customer_password', 'customer_fullname', 'customer_contact');

// Start building query according to input parameters
// If search string
if ($search_str) {
    $db->where('customer_email', '%' . $search_str . '%', 'like');
    $db->orwhere('customer_fullname', '%' . $search_str . '%', 'like');
}
    ...
    <div class="well text-center filter-form">
        <form class="form form-inline" action="">
            ...
            <select name="order_by" class="form-control">
                <?php
                foreach ($customers->setOrderingValues() as $opt_value => $opt_name) : ($order_by === $opt_value) ? $selected = 'selected' : $selected = '';
